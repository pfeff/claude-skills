...
| **vault-cloud** | `/mbp:vault-cloud [search\|write]` | Record and search the Obsidian vault from a **cloud** session (Claude Code on the web), where the macOS `obsidian` CLI is unavailable but the vault is reachable as a git repo. Clones the vault, searches it with ripgrep (lexical — QMD/semantic search stays on the Mac), and writes vocabulary-compliant notes (validated against the vault's `_meta/` contract) that it commits to a dedicated `cloud/<date>-<topic>` branch for the operator to merge. The cloud mirror of `compound`/`kb-*`. |
