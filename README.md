...
| **scan-document** | `/mbp:scan-document <file>` | Front door for scanned documents: OCRs a PDF or image natively on macOS (Vision framework — no tesseract/poppler), classifies the document from the recognized text, then routes it to a handler skill (e.g. `medical-records`) or a generic vault capture. Owns the reusable `ocr.swift` primitive. |
